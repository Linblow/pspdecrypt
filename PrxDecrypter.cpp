...
/* PSOne Classics DATA.PSP executable, requires secure ID (ie. the PGDs vkey). */
static const u8 pops_0DAA10F0_key[] = {0xa9,0x81,0x71,0x9d,0x92,0x2d,0xcc,0xee,0x44,0x1c,0x0e,0x37,0x7a,0xf6,0xe2,0x3e};
static const u8 pops_0DAA10F0_key2[] = {0xd3,0x43,0xa8,0x49,0x79,0x61,0x82,0x63,0x40,0xbf,0xa3,0xef,0xb0,0x99,0xed,0x48};
static const u8 pops_0DAA06F0_key[] = {0xca,0x26,0x7d,0xa2,0xb9,0xce,0x24,0x6e,0xfd,0x32,0xa8,0x97,0xf4,0x7c,0x19,0x19};
static const u8 pops_0DAA06F0_key2[] = {0x77,0x32,0x20,0x31,0xdf,0x7f,0x4b,0x1c,0x8d,0xd7,0xd2,0xc3,0x23,0xa9,0xf8,0xa9};

/* These 6 tags/keys *can* be changed at runtime via OPNSSMP.BIN */
// D91611F0
static u8 resmgr_eboot1_key[]  = {0x61,0xb0,0xc0,0x58,0x71,0x57,0xd9,0xfa,0x74,0x67,0x0e,0x5c,0x7e,0x6e,0x95,0xb9};
// D91680F0
static u8 resmgr_eboot2_key[]  = {0x2c,0x22,0x9b,0x12,0x36,0x74,0x11,0x67,0x49,0xd1,0xd1,0x88,0x92,0xf6,0xa1,0xd8};
// D91690F0
static u8 resmgr_eboot3_key[]  = {0x42,0x61,0xe2,0x57,0x94,0x49,0x42,0xb5,0xaa,0x6d,0x0d,0x08,0x3d,0x24,0xf7,0x4b};
// 2E5E11F0
static u8 resmgr_pboot1_key[]  = {0x75,0xeb,0xe8,0x43,0xf4,0x87,0x8f,0xd0,0x14,0x7f,0x7e,0x39,0xad,0xaf,0x04,0x9d};
// 2E5E80F0
static u8 resmgr_pboot2_key[]  = {0x0f,0x74,0xaf,0x43,0x75,0xcd,0xda,0x39,0x81,0x56,0xd9,0x61,0x3e,0x16,0xc8,0x92};
// 2E5E90F0
static u8 resmgr_pboot3_key[]  = {0x67,0xe4,0x8f,0x4c,0x08,0xa0,0x7d,0xb1,0x5f,0x51,0xa7,0x72,0x98,0xa8,0x2d,0x7e};
/* Resmgr PBOOT tags have a constant secondary key though. */
static const u8 resmgr_pboot_key2[] = {0x69,0xba,0x55,0x34,0xf0,0xc0,0xd6,0x71,0xe3,0x1f,0xdb,0x97,0xe0,0x7c,0xd2,0x2a};
...
/* Resmgr tags *can* be changed at runtime prior to game boot via OPNSSMP.BIN */
static TAG_INFO2 g_resmgrTagInfo2[] =
{
	{ 0x2E5E11F0, resmgr_pboot1_key, 0x48, 5,  resmgr_pboot_key2 },
	{ 0x2E5E80F0, resmgr_pboot2_key, 0x48, 7,  resmgr_pboot_key2 },
	{ 0x2E5E90F0, resmgr_pboot3_key, 0x48, 10, resmgr_pboot_key2 },
	{ 0xD91611F0, resmgr_eboot1_key, 0x5d, 2 },
	{ 0xD91680F0, resmgr_eboot2_key, 0x5d, 6 },
	{ 0xD91690F0, resmgr_eboot3_key, 0x5d, 9 },
};
//TODO add support for overwriting the resmgr tags by the ones in OPNSSMP.BIN
...
static const TAG_INFO2 g_tagInfo2[] =
{
	...
	{ 0x0DAA10F0, pops_0DAA10F0_key, 0x65, 5, pops_0DAA10F0_key2 }, // PSOne Classics DATA.PSP, requires secure ID (PGDs vkey)
	{ 0x0DAA06F0, pops_0DAA06F0_key, 0x65, 5, pops_0DAA06F0_key2 }, // PSOne Classics DATA.PSP, requires secure ID (PGDs vkey)
	{ 0x2E5E10F0, key_2E5E10F0, 0x48, 5, resmgr_pboot_key2 }, // PBOOT constant tag & key
	{ 0x2E5E12F0, key_2E5E12F0, 0x48, 5 }, // resmgr tag that normally replaces 0x2E5E11F0 tag/key
	{ 0x2E5E13F0, key_2E5E13F0, 0x48, 5 }, // resmgr tag that normally replaces 0x2E5E11F0 tag/key
...
static const TAG_INFO2 *GetTagInfo2(u32 tagFind)
{
	...
	for (u32 iTag = 0; iTag < sizeof(g_resmgrTagInfo2) / sizeof(TAG_INFO2); iTag++)
	{
		if (g_resmgrTagInfo2[iTag].tag == tagFind)
		{
			return const_cast<const TAG_INFO2 *>(&g_resmgrTagInfo2[iTag]);
		}
	}
...
static std::array<u8, 0x90> expandSeed(const u8 *seed, int key, const u8 *secureId = nullptr)
{
	...
	if (secureId)
	{
		...
		{
			expandedSeed[i] ^= secureId[i % 0x10];
		}
	}
...
struct PRXType5
{
	...
	void decrypt(int key, const u8 *xor1, const u8 *secureId)
	{
...
static int pspDecryptType5(const u8 *inbuf, u8 *outbuf, u32 size, const u8 *secureId)
{
	//INFO_LOG(LOADER, "Decrypting tag %08X", pspGetTagVal(inbuf));
	const auto compSize = pspGetCompSize(inbuf);
	const auto pti = GetTagInfo2(pspGetTagVal(inbuf));
	...
	// expand the seed into a xor buffer
	auto xorbuf = expandSeed(pti->key, pti->code, secureId);

	// construct the header format for a type 2 prx
	PRXType5 type5(inbuf);
	type5.decrypt(pti->code, pti->seed, secureId);
...
int pspDecryptPRX(const u8 *inbuf, u8 *outbuf, u32 size, const u8 *secureId, bool verbose)
{
	...
	res = pspDecryptType5(inbuf, outbuf, size, secureId);
	if (res >= 0) {
		type = 5;
		goto end;
	}
